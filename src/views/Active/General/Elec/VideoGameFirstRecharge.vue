...
import BannerImg from "@/assets/img/active/elec/FirstRecharge/wap-banner.jpg"
...
class ActiveGetVideoGameFirstRechargeInit extends ActiveGetChessCardsFirstRechargeInit {}

const initData = ref<ActiveGetVideoGameFirstRechargeInit>(new ActiveGetVideoGameFirstRechargeInit())
...
const walletList = ref<ActiveGetVideoGameFirstRechargeInit["walletList"]>([])

const init = async (): Promise<void> => {
    systemParams.PageLoading = true
    HttpPlus.setUrlOptions(ApiSetting.active.Elec2022.FirstRechargeInit)
    const res = await HttpPlus.sendHttpRequest<ActiveGetVideoGameFirstRechargeInit>()
...
<template>
    <ActiveUniversalTem
        class="VideoGameFirstRecharge-Page"
        :headTitle="'电玩首存优惠'"
        :BannerImg="BannerImg"
        :time="initData.active_show_time"
        :initData="initData"
        :walletList="walletList"
        :desc="`活动期间，新会员首次存款时可申请一次电玩首存优惠，申请成功的用户可享受单笔存款最高${initData.active_setting?.rate}的存款优惠。`"
        :btnText="btnText"
