...
import BannerImg from "@/assets/img/active/chess/FirstRecharge/wap-banner.png"
...
const initData = ref<ActiveGetChessCardsFirstRechargeInit>(new ActiveGetChessCardsFirstRechargeInit())
...
const walletList = ref<ActiveGetChessCardsFirstRechargeInit["walletList"]>([])

const init = async (): Promise<void> => {
    systemParams.PageLoading = true
    HttpPlus.setUrlOptions(ApiSetting.active.Chess2022.FirstRechargeInit)
    const res: any = await HttpPlus.sendHttpRequest<ActiveGetChessCardsFirstRechargeInit>()
...
<template>
    <ActiveUniversalTem
        class="ChessCardsFirstRecharge-Page"
        :headTitle="'棋牌首存优惠'"
        :BannerImg="BannerImg"
        :time="initData.active_show_time"
        :initData="initData"
        :walletList="walletList"
        :desc="`活动期间，新会员首次存款时可申请一次棋牌首存优惠，申请成功的用户可享受单笔存款最高${initData.active_setting?.rate}的存款优惠。`"
        :btnText="btnText"
    ...
    >
        ...
        <template #rules>
            <ol>
                <li>
                    符合条件的会员可在点击本优惠活动页面上“立即首存”按钮后，选择“乐游棋牌钱包”、“开元棋牌钱包”、“博乐棋牌钱包”、“凯旋棋牌钱包”其中之一进行首存转账。对应的存款本金+活动奖金在转账成功后将自动添加至用户选择的棋牌游戏钱包中。
                </li>
