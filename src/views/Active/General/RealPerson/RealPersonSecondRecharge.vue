...
import BannerImg from "@/assets/img/active/realman/RealPersonSecondRecharge/wap-banner.jpg"
...
class ActiveGetRealPersonSecondRechargeInit extends ActiveGetChessCardsFirstRechargeInit {
    active_setting?: {
        limit: number
...
const initData = ref<ActiveGetRealPersonSecondRechargeInit>(new ActiveGetRealPersonSecondRechargeInit())
...
const walletList = ref<ActiveGetRealPersonSecondRechargeInit["walletList"]>([])

const init = async (): Promise<void> => {
    systemParams.PageLoading = true
    HttpPlus.setUrlOptions(ApiSetting.active.RealPerson2022.SecondRechargeInit)
    const res = await HttpPlus.sendHttpRequest<ActiveGetRealPersonSecondRechargeInit>()
...
<template>
    <ActiveUniversalTem
        class="RealPersonSecondRecharge-Page"
        :headTitle="'真人复存优惠'"
        :BannerImg="BannerImg"
        :time="initData.active_show_time"
        :initData="initData"
        :walletList="walletList"
        :desc="`活动期间，新会员完成注册后的第二笔存款，即可申请一次真人复存优惠，申请成功的用户可享受单笔存款最高${initData.active_setting?.rate}的存款优惠。`"
        :btnText="btnText"
    ...
    >
        ...
        <template #rules>
            <ol>
                ...
                <li>
                    此活动复存定义为：用户注册后的第二笔存款；若用户在申请前进行转账操作或未完成真人首存活动的流水要求，则不可申请复存优惠。
                </li>
                <li>
